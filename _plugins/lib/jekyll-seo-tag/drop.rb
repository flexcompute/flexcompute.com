...
module Jekyll
  class SeoTag
    class Drop < Jekyll::Drops::Drop
      ...
      SEO_TITLE_SEPARATOR = " | "
      SEO_FORMAT_STRING_METHODS = [
        :markdownify, :strip_html, :normalize_whitespace, :escape_once,
      ].freeze
      SEO_HOMEPAGE_OR_ABOUT_REGEX = %r!^/(about/)?(index.html?)?$!.freeze

      SEO_EMPTY_READ_ONLY_HASH = {}.freeze
      private_constant :SEO_EMPTY_READ_ONLY_HASH

      def initialize(text, context)
        @obj = SEO_EMPTY_READ_ONLY_HASH
        @mutations = {}
        @text = text
        @context = context
      end
      ...
      def title
        @title ||= begin
          if site_title && page_title != site_title
            page_title + SEO_TITLE_SEPARATOR + site_title
          elsif site_description && site_title
            site_title + SEO_TITLE_SEPARATOR + site_tagline_or_description
          else
      ...
      def description
        if page['pagination_blog']
            @description = false
        else
         @description ||= begin
              format_string(page["description"] || page["excerpt"]) || site_description || title
            end
        end
      end
      ...
      def homepage_or_about?
        page["url"] =~ SEO_HOMEPAGE_OR_ABOUT_REGEX
      end
      ...
      def format_string(string)
        string = SEO_FORMAT_STRING_METHODS.reduce(string) do |memo, method|
